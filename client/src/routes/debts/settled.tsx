    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-gray-600 dark:text-gray-400">Historial de deudas que ya fueron saldadas</p>
        </div>
      </div>
      ...
      {debts?.length === 0 && (
        <div className="rounded-xl border-2 border-dashed border-gray-300 bg-white p-12 text-center dark:border-gray-600 dark:bg-gray-800">
          <CheckCircle2 className="mx-auto h-12 w-12 text-gray-500 dark:text-gray-500" />
          <p className="mt-4 text-gray-600 dark:text-gray-400">{tab === "creditor" ? "No te han pagado deudas aun" : "No has pagado deudas aun"}</p>
        </div>
      )}
          ...
          >
            <div className="flex items-center justify-between gap-2">
              ...
              <div className="shrink-0 text-right">
                ...
                <p className="text-xs text-gray-500 dark:text-gray-500">Liquidada el {formatDate(debt.updatedAt)}</p>
              </div>
            </div>
          </button>
        ))}
